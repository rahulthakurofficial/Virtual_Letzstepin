...
const ImageComparisonSlider = ({ beforeSrc, afterSrc, price }) => {
  ...
  const handleMouseMove = (event) => {
    ...
    handle.style.left = `calc(${((mouseX / sliderWidth) * 100).toFixed(4)}% - ${handleWidth / 2}px)`;
  };
  ...
  return (
    <div className="comparison-box">
      <div className="image-comparison-slider" ref={sliderRef}
      ...
      <div className="price-tag">Price: {price}</div> {/* Price Added */}
    </div>
  );
};
...
const TwoD_Staging = () => {
  return (
    <div className="outer-box">
      <h2 className="staging-heading">Revamp Your Home – Visualize the Difference!</h2>
      <div className="inner-box">
        <div className="grid-container">
          {beforeImages.map((before, index) => (
            <ImageComparisonSlider key={index} beforeSrc={before} afterSrc={afterImages[index]} price={prices[index]} />
          ))}
        </div>
      </div>
    </div>
